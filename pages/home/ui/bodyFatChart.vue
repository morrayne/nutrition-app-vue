...
const getPercentages = (logs: tWeightLog[]) => {
  ...
  const bodyFats = last10.map((log) => log.bodyFat).filter((f) => f !== undefined) as number[];
  ...
  return last10.map((log) => ({
    id: log.id,
    date: log.date,
    percent: log.bodyFat !== undefined ? calculatePercent(log.bodyFat, minBodyFat, maxBodyFat) : 50,
    value: log.bodyFat,
  }));
};
