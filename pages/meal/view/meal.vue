...
<template>
  ...
  <div class="grid grid-3 ali-c gap-50" v-if="topMode === 'add' && botMode === 'newMeal'">
    <vInputString v-model="basket.name" :construct="mealName" />
    <p class="w-100 jus-c text-m bounce-m main" @click="finish">{{ t("finish") }}</p>
  </div>
  <div class="w-100 grid grid-3 gap-50" v-if="topMode === 'view'">
    <mealWrap v-for="item in mealStore.meals" :construct="item" />
  </div>
  <div class="w-100 grid grid-3 gap-50" v-if="topMode === 'add' && botMode === 'newMeal'">
  ...
  <div class="w-100 grid grid-3 gap-50" v-if="topMode === 'add' && botMode === 'products'">
