...
import { ref } from "vue";
import type { tInput } from "../../appSettings/types/form";
...
const handleInput = (event: Event) => {
  ...
  if (dataType === "number") {
    ...
    if (normalized.includes('.') && normalized.split('.')[1] === '') {
      value = rawValue;
      hasError.value = true;
    } else {
...
const checkForErrors = (data: string | number, mode: "string" | "number" | "password") => {
  hasError.value = false;
  hasCorrect.value = false;
  if (data === '' || data === null || data === undefined) return;
  if (mode === "number") {
    const num = Number(data);
    const rules = props.construct.rule.number;
    if (isNaN(num)) {
      hasError.value = true;
      return;
    }
    ...
    const rules = props.construct.rule.string;
...
<template> 
  <div class="flex-c g-05 w-100">
    ...
    <input :name="props.construct.title" :disabled="props.disable" :type="dataType === 'password' ? 'password' : 'text'" class="w-100 def-wrap" :class="[hasError ? 'error' : '', hasCorrect ? 'correct' : '']" :placeholder="placeholder" @input="handleInput" :value="modelValue" :style="props.undercover ? 'padding: 0.5rem 0.75rem' : ''" />
  </div>
</template>
